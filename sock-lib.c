...
int conectar (int argc, char * argv[])
{
	...
	int	sockfd;
	struct hostent *he;	/* Se utiliza para convertir el nombre del host a su direcci�n IP */
...
/* Establecemos their_addr con la direccion del server */
	...
	their_addr.sin_port = (argc == 2)? htons(PORT):htons(atoi(argv[2])); /*SI NO SE PAS� EL PUERTO AL CUAL CONECTARSE COMO 2do PAR�METRO EN argv, SE UTILIZA EL ESTABLECIDO EN EL #DEFINE DE sock-lib.h*/
...
/*Crea un socket y verifica si hubo alg�n error*/
	...
	{
		fprintf(stderr, "Error en funci�n socket. Código de error %s\n", strerror(sockaux));
		return -1;
	}
...
int Aceptar_pedidos (int sockfd, struct sockaddr_in *aux)
{
