...
void *threadFunctionClient(void *tpc_data)
{
      char bufTCP[MAXDATASIZE_TCP];
      int numbytes;
      threadParamsClient *tpc = (threadParamsClient *) (tpc_data);
       while(1)
       {
...
void *threadFunctionServer(void *tp_data)
{
      char i;
      struct timespec t;
      int sockudp;
      threadParamsServer *tp = (threadParamsServer *) (tp_data);
...
int main (int argc, char *argv[])
{
  ...
  if (argc < 2)
  {
    fprintf(stderr,"Usage: ./video server or ./video [host ip] [port] [width] [height] [fps] [udp_port] [local_ip]\n");
    exit(1);
  }
  ...
  if (argc == 2)
  {
        ...
        while(1)
        {
                  ...
                  switch(pid = fork())
                  {
                          ...
                          case 0:
                                ...
                                pthread_create (&thread, &attr, threadFunctionServer, (void *)&tp);
    ...
    if(argc == 8)
    {
            ...
            tpc.fd=sockfd;
            pthread_attr_init(&attr);
            pthread_create (&thread, &attr, threadFunctionClient, (void*)&tpc);
            ...
            my_addr.sin_addr.s_addr=inet_addr(argv[7]);
            my_addr.sin_port=htons(atoi(argv[6]));
            if (bind(sockudp,(struct sockaddr *)&my_addr,sizeof(my_addr)) == -1)
            {
              perror("Error bind");
              exit(1);
            }
